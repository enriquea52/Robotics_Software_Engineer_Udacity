...
// Define a position and orientation for the robot to reach DROP ZONE
  goal.target_pose.pose.position.x = -2.0;
  ...
  // Check if the robot reached its goal
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
    ROS_INFO("Hooray, the base moved to the goal zone");
  else
    ROS_INFO("The base failed to move to the goal zone for some reason");
